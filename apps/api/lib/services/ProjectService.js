...
class ProjectService {
  ...
  async healthCheck() {
    try {
      // Usar un health check más simple que no dependa de stats completas
      const basicCheck = await this.repository.count();
      const isHealthy = typeof basicCheck === 'number' && basicCheck >= 0;
      
      if (isHealthy) {
        // Si la conexión básica funciona, intentar obtener stats
        try {
          ...
          logger.warn('Error obteniendo stats completas en health check', statsError);
          return {
            healthy: true,
            stats: { total: basicCheck },
            warning: 'Stats limitadas disponibles',
            lastCheck: new Date().toISOString()
          };
        }
      }
      ...
      // En producción, no fallar el health check por errores temporales de conexión
      if (error.code === 'PROTOCOL_PACKETS_OUT_OF_ORDER' || 
          error.code === 'PROTOCOL_CONNECTION_LOST' ||
          error.code === 'PROTOCOL_ENQUEUE_AFTER_DESTROY') {
        logger.warn('Error temporal de conexión MySQL en health check, reintentando...');
        
        // Esperar un poco y reintentar una vez
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        try {
          const retryCheck = await this.repository.count();
          return {
            healthy: typeof retryCheck === 'number',
            stats: { total: retryCheck || 0 },
            warning: 'Recuperado de error temporal de conexión',
            lastCheck: new Date().toISOString()
          };
        } catch (retryError) {
          logger.errorWithStack('Error en reintento de health check', retryError);
        }
      }
