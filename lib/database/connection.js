  ...
  connect() {
    try {
      ...
      this.pool.on('error', (error) => {
        ...
        // Para errores de protocolo, NO cerrar el pool inmediatamente
        if (error.code === 'PROTOCOL_CONNECTION_LOST' || 
            error.code === 'PROTOCOL_PACKETS_OUT_OF_ORDER' ||
            error.code === 'PROTOCOL_ENQUEUE_AFTER_DESTROY') {
          logger.warn('Error de protocolo detectado - continuando con pool existente');
          // NO llamar a reconnect() inmediatamente para evitar cierre del pool
          // El pool se autoreparará con nuevas conexiones
        }
      });
  ...
  async query(sql, params = []) {
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        // Reintentar si es un error de protocolo y no hemos agotado los reintentos
