...
class UserRepository {
  ...
  async create(user) {
    try {
      // Usar validación específica para configuración inicial si no tiene contraseña de Workana
      const validation = user.workanaPassword ? user.validate() : user.validateForInitialSetup();
      ...
      if (data.system_password) {
        ...
        data.system_password = await bcrypt.hash(data.system_password, 10);
      }
