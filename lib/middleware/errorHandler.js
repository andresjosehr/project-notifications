...
class ErrorHandler {
  ...
  setupGlobalErrorHandlers() {
    ...
    process.on('unhandledRejection', (reason) => {
      ...
      // Si es un error de base de datos crítico, cerrar gracefully
      if (reason && reason.code === 'PROTOCOL_PACKETS_OUT_OF_ORDER') {
        logger.warn('Error crítico de base de datos detectado - iniciando cierre graceful');
    ...
    process.on('uncaughtException', (error) => {
      ...
      // Para errores de base de datos, NO llamar handleError para evitar recursión
      if (error.code === 'PROTOCOL_PACKETS_OUT_OF_ORDER' || 
          error.code === 'PROTOCOL_ENQUEUE_AFTER_DESTROY' ||
          error.code === 'PROTOCOL_CONNECTION_LOST' ||
          error.code === 'POOL_CLOSED') {
        logger.warn('Error crítico de protocolo MySQL detectado - iniciando cierre graceful sin notificación');
        this.gracefulShutdownWithoutNotification();
      } else {
    ...
    process.on('SIGTERM', () => {
      logger.shutdownLog('SIGTERM received, shutting down gracefully...');
      this.gracefulShutdown();
    });
    
    process.on('SIGINT', () => {
      logger.shutdownLog('SIGINT received, shutting down gracefully...');
      this.gracefulShutdown();
    });
  }
