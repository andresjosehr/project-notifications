...
class AuthMiddleware {
  ...
  authenticate = async (req, res, next) => {
    try {
      ...
      const [rows] = await db.execute(
        'SELECT id, workana_email, role, is_active FROM users WHERE id = ? AND is_active = 1',
        [decoded.userId]
      );
  ...
  async login(email, password) {
    try {
      const db = await this.initDB();
      const [rows] = await db.execute(
        'SELECT id, workana_email, system_password, role, is_active FROM users WHERE workana_email = ? AND is_active = 1',
        [email]
      );
      ...
      const isValidPassword = await this.verifyPassword(password, user.system_password);
      if (!isValidPassword) {
        throw new Error('Invalid credentials');
      }
