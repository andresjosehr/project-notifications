...
class ProjectController {
  ...
  async runScrapingCycle(options = {}) {
    try {
      ...
      // Comentado: No enviar notificaciones de estado/resumen
  ...
  // Método comentado: No enviar notificaciones de resumen de ciclo
  ...
  async healthCheckWithoutAI() {
    try {
      ...
      // Health check de servicios (sin IA)
      results.services.projects = await this.projectService.healthCheck();
      results.services.notifications = await this.notificationService.healthCheck();

      // Health check general sin validar IA
      const allHealthy = Object.values(results.services).every(service => service.healthy);
      ...
      logger.info('Health check completado sin validación de IA', {
        projects: results.services.projects?.healthy,
        notifications: results.services.notifications?.healthy,
        overall: results.overall.status
      });

      return results;
    } catch (error) {
      logger.errorWithStack('Error en health check sin IA', error);
      return {
        timestamp: new Date().toISOString(),
        overall: {
          healthy: false,
