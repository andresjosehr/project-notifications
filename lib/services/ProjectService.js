...
class ProjectService {
  ...
  async processNewProjects(projects, platform, options = {}) {
    try {
      ...
      logger.info(`La opcion de enviar notificaciones es: ${options.sendNotifications}, ademas la iteracion es: ${options.iteration}`);
      if (options.sendNotifications !== false && options.iteration > 0) {
        logger.info(`Enviando notificaciones para ${newProjects.length} proyectos nuevos`);
