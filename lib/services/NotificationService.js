...
class NotificationService {
  ...
  async sendTranslatedProjectNotification(project, user = null, options = {}) {
    try {
      ...
      // Agregar información específica según la plataforma
      if (project.platform === 'workana') {
        originalMessage += `💰 Precio: ${project.price}\n\n`;
      } else if (project.platform === 'upwork') {
        originalMessage += `ℹ️ Info: ${project.info}\n\n`;
      }
      
      originalMessage += `📋 Título:\n${project.title}\n\n`;
      originalMessage += `📝 Descripción:\n${project.description}`;
      ...
      if (project.id) {
        ...
        translatedMessage += `📄 *Generar propuesta:*\n${proposalUrl}`;
      }
