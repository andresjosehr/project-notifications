...
class WorkanaService {
  ...
  async sendProposalWithCustomContent(projectId, userId, proposalContent, options = {}) {
    try {
      ...
      const proposalRepo = new UserProposalRepository();
      const existingProposal = await proposalRepo.findByProjectAndUser(projectId, userId);
      ...
      if (!hasSession) {
        return {
          success: false,
          error: 'No se pudo establecer sesión con Workana'
        };
      }
      ...
      const projectUrl = `${this.baseUrl}/jobs/${projectId}`;
      ...
      // Buscar el botón de enviar propuesta
      const submitButton = await this.page.$('[data-testid="apply-project-button"], .btn-primary:has-text("Enviar propuesta"), .btn:has-text("Enviar propuesta")');
      
      if (!submitButton) {
        return {
          success: false,
          error: 'No se encontró el botón para enviar propuesta'
        };
      }
      ...
      // Buscar el campo de texto para la propuesta
      const proposalField = await this.page.$('textarea[name="message"], textarea[placeholder*="propuesta"], textarea[placeholder*="mensaje"]');
      ...
      // Limpiar el campo y escribir la propuesta personalizada
      await proposalField.selectText();
      await proposalField.fill(proposalContent);
      await this.page.waitForTimeout(1000);

      // Buscar y hacer clic en el botón de envío final
      const finalSubmitButton = await this.page.$('button[type="submit"], button:has-text("Enviar"), .btn-primary:has-text("Enviar")');
