...
class WorkanaService {
  ...
  async _saveUserSession(userId, userEmail) {
    try {
      ...
      // Guardar sesión en external_credentials
      const externalCredentialRepository = new ExternalCredentialRepository();
      await externalCredentialRepository.updateSessionData(userId, sessionDataJson, expiresAt);
      logger.info(`Sesión guardada en external_credentials para usuario ${userEmail}`);
    } catch (dbError) {
      logger.errorWithStack('Error guardando sesión en external_credentials', dbError);
    }
  }
