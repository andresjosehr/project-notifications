...
class WorkanaService {
  ...
  async sendProposal(projectId, options = {}) {
    try {
      ...
      // Verificar si ya se envió una propuesta para este proyecto
      try {
        const rows = await query(
          'SELECT proposal_sent_at FROM workana_projects WHERE id = ?',
          [projectId]
        );
        
        if (rows.length > 0 && rows[0].proposal_sent_at) {
          throw new Error(`Ya se envió una propuesta para este proyecto el ${rows[0].proposal_sent_at}`);
        }
      } catch (error) {
        if (error.message.includes('Ya se envió una propuesta')) {
          throw error;
        }
        logger.warn('Error verificando propuesta previa:', error);
      }
      ...
      try {
        const rows = await query(
          'SELECT link, title, description FROM workana_projects WHERE id = ?',
          [projectId]
        );
      ...
      // Generar propuesta con IA usando el perfil profesional y las directrices
      let proposalText;
      try {
        ...
        logger.info('Generando propuesta con IA...');
        proposalText = await aiService.generateProposalWithProfile(
      ...
      // Actualizar la columna proposal_sent_at en la base de datos
      try {
        await query(
          'UPDATE workana_projects SET proposal_sent_at = NOW() WHERE id = ?',
          [projectId]
        );
        logger.info(`Columna proposal_sent_at actualizada para proyecto ${projectId}`);
      } catch (updateError) {
        logger.errorWithStack('Error actualizando proposal_sent_at', updateError);
      }
      ...
      return {
        ...
        message: 'Propuesta generada y guardada en debug.log (modo prueba)',
